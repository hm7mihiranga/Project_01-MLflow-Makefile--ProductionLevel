...
from handle_missing_values import DropMissingValuesStrategy, CleanNumericColumnStrategy, DropColumnStrategy
from outlier_detection import OutlierDetector, IQROutlierDetection
from feature_binning import TenureBinnerStrategy
from feature_encoding import BinaryEncodingStrategy, MultiCategoryEncodingStrategy
...
def data_pipeline(
    ...
    drop_handler = DropColumnStrategy(column_name=columns['drop_columns'][0]) 
    total_charges_handler = CleanNumericColumnStrategy(column_name=columns['critical_columns'][0]) 
    ...
    if 'feature_binning' in binning_config:
        tenure_config = binning_config['feature_binning']['tenure_category']
    elif 'tenure_category' in binning_config:
        tenure_config = binning_config['tenure_category']
    else:
        tenure_config = {
            'New': [0, 12],
            'Established': [13, 48],
    ...
    for label, range_vals in tenure_config.items():
        if range_vals[1] is not None:  
            bins.append(range_vals[1])
        labels.append(label)
    
    if bins:
        bins.append(float('inf'))
    
    binning = TenureBinnerStrategy(bins=bins, labels=labels)
    df = binning.bin_feature(df, 'tenure')
    print(f"data after feature binning: \n{df.head()}")

    print('\nStep 5: Feature Encoding')

    nominal_strategy = BinaryEncodingStrategy(columns['bin_cols'])
    ordinal_strategy = MultiCategoryEncodingStrategy(columns['multi_cols'])
    ...
    StandardScaling_strategy = StandardScalingStrategy()
    df = StandardScaling_strategy.scale(df, scaling_config['columns_to_scale'])
